...
export const ACTION = {
  PAGINATE: {
    ADDED(payload: Notice, query: PaginateQuerySearch) {
    ...
    UPDATE(payload: Notice, query: PaginateQuerySearch) {
      TanstackQuery.setQueryData<PaginateMetaResponse<Notice[]>>(
        [QUERY.NOTICE_PAGINATE, query],
        (old) => {
          if (!old) {
  ...
  SHOW: {
    UPDATE(payload: Notice) {
      TanstackQuery.setQueryData<Notice>(
        [QUERY.NOTICE_SHOW, payload.id],
        (old) => {
