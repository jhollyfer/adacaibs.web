...
import { addedVideoToPagination } from "@/lib/tanstack/actions/videos";
import { useVideoCreateMutation } from "@/lib/tanstack/mutation/videos/create";
import { VideoCreatePayload, VideoSchema } from "@/schemas/videos";
...
export function Form({ onClose }: { onClose: () => void }): React.JSX.Element {
  ...
  const create = useVideoCreateMutation({
    onError(error) {
      console.log(error);
    },
    onSuccess(response) {
      addedVideoToPagination(response, {
  ...
  const form = useForm<VideoCreatePayload>({
    resolver: zodResolver(VideoSchema["create"]),
  });
